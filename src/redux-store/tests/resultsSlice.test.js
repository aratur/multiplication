...
describe('Store - results', () => {
  it('setValueAtRowCol throws on duration equal to zero', () => {
    const durationEqualZero = () =>
      setStatusDurationAtRowCol(resultStatus.pending, 0, 1, 1);
  ...
  it('setValueAtRowCol throws on undefined state', () => {
    const undefinedStatus = () => setStatusDurationAtRowCol(undefined, 1, 1, 1);
    expect(undefinedStatus).toThrow(/undefined/);
  });

  it('setValueAtRowCol throws on out of bounds', () => {
    const parametersOutsideOfBounds = () =>
      setStatusDurationAtRowCol(resultStatus.success, 1, 11, 12);
  ...
  it('should generate object 10 x 10 with default values', () => {
    ...
    expect(resultValues[size][size]).toStrictEqual(defaultValue);
    expect(resultValues[1][1]).toStrictEqual(defaultValue);
  });
  ...
  it('getValueAtRowCol should modify value at proper coordinates', () => {
    ...
    expect(store.getState().results.values[xValue][yValue]).toStrictEqual({
      status,
      duration,
    });
  });
  ...
  it('should increment stars count and clear results after the table is solved', () => {
    ...
    iterate.forEach((a) => {
      iterate.forEach((b) => {
        ...
        setStatusDurationAtRowCol(resultStatus.success, 100, a, b);
      });
    });
